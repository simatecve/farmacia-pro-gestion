...
WITH category_ids AS (
  SELECT id, name FROM categories WHERE name IN ('Medicamentos', 'Cuidado Personal', 'Vitaminas', 'Primeros Auxilios')
)
...
SELECT 
    p.name, p.description, p.sku, p.barcode, p.sale_price::numeric, p.purchase_price::numeric, 
    p.min_stock::integer, p.max_stock::integer, p.unit_type, p.presentation, p.concentration, p.laboratory, p.active::boolean,
    c.id
FROM (VALUES
...
WITH location_data AS (
  SELECT id FROM locations WHERE name = 'Farmacia Principal' LIMIT 1
)
